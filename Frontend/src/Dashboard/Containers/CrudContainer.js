...
class CrudContainer extends React.Component {
  ...
  componentDidMount() {
    ...
    const new_fields = this.props.fields.map(field => {
      if (field.inputProps.type === 'text' || field.inputProps.type === 'link') {
      ...
      if (field.inputProps.type === 'select') {
        field.onChange = this.handleSelectChange
      }
      return field;
    })
    ...
    this.setState({
      ...
      if (this.props.initialItems.length > 0) {
        this.getAllData(this.checkInitialData)
      } else {
        this.getAllData()
      }
    });
  ...
  checkInitialData = () => {
    ...
    this.props.initialItems.map(async initialItem => {
      const { data, search } = initialItem
      try {
        let getParameters = ''
        ...
        const response = await this.callAPI({
          apiURL: `${this.state.apiBaseURL}/search?${getParameters}`,
          method: 'GET',
        })
        ...
        if (response.data.status.code === 200) {
          ...
          if (response.data.data.length === 0) {
            await this.addNewDataWithPredefinedInputs(data)
          }
        } else {
  ...
  addNewDataWithPredefinedInputs = async (data) => {
    try {
      const apiDataHeaders = {}
      apiDataHeaders.data = {}
      Object.keys(data).forEach(key => {
        apiDataHeaders.data[key] = data[key]
      })
